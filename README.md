...
Repos are cloned to `~/.agents/{rules,skills,docs,commands}/usergenerated` and updated hourly via cron (Linux/macOS) or scheduled task (Windows). Run the update script manually any time:

```bash
~/.agents/update-usergenerated.sh
```

```powershell
~/.agents/update-usergenerated.ps1
```
